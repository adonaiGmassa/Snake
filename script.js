...
function Nourriture() { 
  ...
  serpent.forEach(function estNourritureSurSerpent(partie) { // Vérifie si la nourriture a été mangée par le serpent
    const nourritureSurSerpent = partie.x == coordonneNourritureX && partie.y == coordonneNourritureY; // Vérifie la collision
    if (nourritureSurSerpent) Nourriture(); // Si la nourriture est sur le serpent, relance la fonction
    }
  );
...
function avancerSerpent() { 
  ...
  // Crée une nouvelle tête pour le serpent
...
function Mouvement(event) { 
  ...
  const ALLANT_HAUT = deplacementY === -10; // Vérifie si le serpent va vers le haut
  const ALLANT_DROITE = deplacementX === 10; // Vérifie si le serpent va vers la droite
  const ALLANT_GAUCHE = deplacementX === -10; // Vérifie si le serpent va vers la gauche
  const ALLANT_BAS = deplacementY === 10; // Vérifie si le serpent va vers le bas

  // Allé vers le haut
  if (TOUCHE_Pressee === HAUT && !ALLANT_BAS) {
    deplacementX = 0; // Pas de mouvement horizontal
    deplacementY = -10; // Change la direction vers le haut
  }
  
  // Allé vers la Gauche
  if (TOUCHE_Pressee === GAUCHE && !ALLANT_DROITE) {
    deplacementX = -10; // Change la direction vers la gauche
    deplacementY = 0; // Pas de mouvement vertical
  }
  // Allé vers la droite
  if (TOUCHE_Pressee === DROITE && !ALLANT_GAUCHE) {
      deplacementX = 10; // Change la direction vers la droite
      deplacementY = 0; // Pas de mouvement vertical
  }
  
  // allé vers le bas 
  if (TOUCHE_Pressee === BAS && !ALLANT_HAUT) {
    deplacementX = 0; // Pas de mouvement horizontal
